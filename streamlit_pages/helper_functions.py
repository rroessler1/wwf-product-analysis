...
from openai_integration.models import FinalProductCategory
...
def get_filtered_page_data(data: pd.DataFrame, current_folder: str, current_page_number: int) -> pd.DataFrame:
...
def show_current_image(current_folder: str, current_page_number: int) -> None:
    ...
    try:
        ...
        st.sidebar.image(image, caption=f"Image for {current_folder}, Page {current_page_number}",
                         use_container_width=True)
    except FileNotFoundError:
        st.sidebar.warning(f"Image not found for {current_folder}, Page {current_page_number}")

def show_edit_options(data: pd.DataFrame, filtered_page_data: pd.DataFrame, result_path: str) -> None:
...
def edit_product_category(index: int, data: pd.DataFrame, storage_path: str) -> None:
    with st.form(key=f"form_{index}"):
        ...
        with col1:
            product_name = st.text_input("Product Name", value=str(data.loc[index, "extracted_product_name"]))
        with col2:
            final_category = st.selectbox(label= "Category", options=category_options, index=category_options.index(data.loc[index, "final_category"]))
...
def edit_product_data(index: int, data: pd.DataFrame, storage_path: str) -> None:
    with st.form(key=f"form_{index}"):
        ...
        with col3:
            original_price = st.number_input("Original Price", value=float(data.loc[index, "extracted_original_price"]), format="%0.2f", step=0.1)
        with col4:
            discount_price = st.number_input("Discount Price", value=float(data.loc[index, "extracted_discount_price"]), format="%0.2f", step=0.1)
        with col5:
            percentage_discount = st.number_input("Percentage Discount",
                                            value=float(data.loc[index, "extracted_percentage_discount"]), format="%0.0f", step=1.0)
...
def show_mark_products_as_checked(data: pd.DataFrame, current_folder: str, current_page_number: int, storage_path: str) -> None:
...
def show_add_missing_product(data: pd.DataFrame, current_folder: str, current_page_number: int, storage_path: str, missing_product_counter:int = 0) -> None:
    if "missing_product_counter" not in st.session_state:
        st.session_state.missing_product_counter = 0

    # Button to show the form
    if st.button("Add missing product", key=f"Missing_product_button_{missing_product_counter}"):
    ...
    if st.session_state.missing_product_counter > missing_product_counter:
        with st.form(key=f"form_missing_product_{missing_product_counter}"):
            ...
            with col2:
                final_category = st.selectbox(label="Category", options=(["Select Category"]+category_options))

            col3, col4, col5 = st.columns(3)
            with col3:
                original_price = st.number_input("Original Price", format="%0.2f",step=0.1)
            with col4:
                discount_price = st.number_input("Discount Price", format="%0.2f", step=0.1)
            with col5:
                percentage_discount = st.number_input("Percentage Discount", format="%0.0f", step=1.0)
            ...
            if submitted:
                ...
                if error_messages:
                    ...
                    if not reference_row.empty:
                        new_product.loc[0, "date_collected"] = reference_row.iloc[0]["date_collected"]
                        new_product.loc[0, "calendar_week"] = reference_row.iloc[0]["calendar_week"]
        ...
        show_add_missing_product(data, current_folder, current_page_number, storage_path, missing_product_counter+1)

def show_page_results_df(data: pd.DataFrame, current_folder: str, current_page_number: int) -> None:
    # Display the updated dataframe after editing
    updated_data = data[get_mask_page_results(data, current_folder, current_page_number)]
...
def show_forward_backward_buttons(max_len: int, position: str) -> None:
    ...
    with col1:
        if st.button("Previous Image", key=f"Previous Image ({position})") and st.session_state.current_page_index > 0:
            st.session_state.current_page_index -= 1
            st.session_state.missing_product_counter = 0

    with col3:
        if st.button("Next Image", key=f"Next Image ({position})") and st.session_state.current_page_index < max_len - 1:
            st.session_state.current_page_index += 1
            st.session_state.missing_product_counter = 0
...
def get_mask_current_image(data: pd.DataFrame, current_folder: str, current_page_number: int) -> pd.Series:
    return (data['extracted_folder'] == current_folder) & (data['extracted_page_number'] == current_page_number)

def get_mask_max_llm_confidence(data: pd.DataFrame) -> pd.Series:
    if isinstance(st.session_state.max_llm_confidence, int) & (not st.session_state.include_checked_products):
        return data['final_certainty'] <= st.session_state.max_llm_confidence
    else:
        return pd.Series([True] * len(data))

def get_mask_core(data: pd.DataFrame) -> pd.Series:
    return (get_mask_bbq_products(data) | get_mask_non_bbq_products(data)) & get_mask_max_llm_confidence(data)

def get_mask_selection(data: pd.DataFrame) -> pd.Series:
    return get_mask_core(data) & get_mask_included_checked_products(data)

def get_mask_page(data: pd.DataFrame, current_folder: str, current_page_number: int) -> pd.Series:
    return get_mask_selection(data) & get_mask_current_image(data, current_folder, current_page_number)

def get_mask_page_results(data: pd.DataFrame, current_folder: str, current_page_number: int) -> pd.Series:
    return get_mask_core(data) & get_mask_current_image(data, current_folder, current_page_number)