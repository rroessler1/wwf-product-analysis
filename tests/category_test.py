...
def check_results(categorized_df: pd.DataFrame) -> pd.DataFrame:
    categorized_df["check_category"] = (
        categorized_df["category"] == categorized_df["solution_category"]
    )
    categorized_df["check_is_grill"] = (
        categorized_df["is_grill"] == categorized_df["solution_is_grill"]
    )
    return categorized_df
...
def show_mismatches(df):
    answer = (
        input("Do you want to see rows with mismatches? (yes/no): ").strip().lower()
    )

    if answer in ["yes", "y"]:
        mismatches = df[
            (df["check_category"] == False) | (df["check_is_grill"] == False)
        ]
        if mismatches.empty:
