...
### 1. `Fleischsorte` Categories
Classify products into one of the following categories based on their main ingredient or label:
- **Geflügel** (Poultry): Chicken wings, thighs, poultry sausages, etc.
- **Schwein** (Pork): Includes pork sausages, spare ribs, pulled pork, etc.
- **Rind** (Beef): Steaks, beef burgers, beef sausages, etc.
- **Gemischt** (Mixed): Products containing multiple types of meat (e.g., mixed sausages).
- **Käse** (Cheese): Includes cheeses specifically intended for grilling (e.g., "Grillkäse").
- **Fisch & Meeresfrüchte** (Fish & Seafood): Grilling-specific fish or seafood products (e.g., skewers, fillets).
- **Vegetarisches/Veganes Ersatzprodukt** (Vegetarian/Vegan Substitute): Plant-based alternatives to meat.
  - **Special Rule**: All **Green Mountain** products belong to this category.
- **Grillgemüse** (Grill Vegetables): Vegetables explicitly labeled for grilling.
...
1. **`certainty_fleischsorte`**: Confidence in assigning `Fleischsorte`.
   - High: Product name explicitly matches a category (90%-100%).
...
**Input**:
1. Coop Rindshackfleisch Schweiz
2. Bell Kalbs-Salsiccia
3. Poulet Flügel BBQ
